...
class PatMarID()(implicit conf: Config, spark: SparkSession) extends Lookup with List {
	...
	override def lookupProcess(dataframe: DataFrame): DataFrame = {
		...
		spark.sql("SELECT od.*, cast(lookup.SUPP_LIST_ID as bigint) as SUPP_LIST_ID_LOOKUP FROM od left outer join lookup on (lookup.SRC_LIST_CD=od.PAT_MAR_ID)")
			.drop("PAT_MAR_ID")
			.withColumnRenamed("SUPP_LIST_ID_LOOKUP", "PAT_MAR_ID")
	}
